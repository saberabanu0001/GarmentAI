...
export type ChatVariant = "worker" | "hr" | "compliance";

type LangLabels = { [K in keyof (typeof copy)["en"]]: string };

const CHAT_CONFIG: Record<
  ChatVariant,
  { apiRole: AppRole; topicsEn: string[]; topicsBn: string[]; showHelpline: boolean }
> = {
  worker: {
    apiRole: "worker",
    topicsEn: [...workerTopicPills],
    topicsBn: [...workerTopicPillsBn],
    showHelpline: true,
  },
  hr: {
    apiRole: "hr_staff",
    topicsEn: ["Leave policy", "Overtime & wages", "Workplace grievance"],
    topicsBn: ["ছুটি নীতি", "ওভারটাইম ও মজুরি", "অভিযোগ প্রক্রিয়া"],
    showHelpline: false,
  },
  compliance: {
    apiRole: "compliance_officer",
    topicsEn: ["Factory audit gap", "Fire safety", "Buyer CBLM"],
    topicsBn: ["কারখানা অডিট", "অগ্নি নিরাপত্তা", "ক্রেতা মান"],
    showHelpline: false,
  },
};

const TEXT_PATCH: Record<ChatVariant, { en: Partial<LangLabels>; bn: Partial<LangLabels> }> = {
  worker: { en: {}, bn: {} },
  hr: {
    en: {
      title: "HR intelligence chat",
      subtitle:
        "Policy, workforce, and audit prep — answers use your indexed documents with HR-level access.",
      askPlaceholder: "Ask about leave, payroll, or HR compliance (English / Bangla)…",
      empty: "Ask a question above. GarmentAI uses HR visibility rules on your knowledge base.",
    },
    bn: {
      title: "এইচআর ইন্টেলিজেন্স চ্যাট",
      subtitle: "নীতি ও কর্মী বিষয় — আপনার জমা নথি অনুযায়ী উত্তর।",
      askPlaceholder: "ছুটি, বেতন, নিয়ম — বাংলা বা ইংরেজিতে জিজ্ঞেস করুন…",
      empty: "উপরে প্রশ্ন করুন।",
    },
  },
  compliance: {
    en: {
      title: "Compliance & audit chat",
      subtitle:
        "Regulatory alignment, buyer standards, and audit context from your document library.",
      askPlaceholder: "Ask about CBLM, safety audits, or regulatory gaps…",
      empty: "Ask above. Responses use compliance-officer access to your sources.",
    },
    bn: {
      title: "কমপ্লায়েন্স ও অডিট চ্যাট",
      subtitle: "নিয়ম, মান ও অডিট — আপনার নথি থেকে।",
      askPlaceholder: "নিরাপত্তা, মান, অডিট — জিজ্ঞেস করুন…",
      empty: "উপরে প্রশ্ন করুন।",
    },
  },
};

export function RagChatPortal({ variant }: { variant: ChatVariant }) {
  ...
  const cfg = CHAT_CONFIG[variant];
  const showExtraWidgets =
    variant === "worker" && process.env.NEXT_PUBLIC_SHOW_WORKER_WIDGETS === "true";

  const t = useMemo(
    () => ({ ...copy[uiLang], ...TEXT_PATCH[variant][uiLang] }),
    [variant, uiLang],
  );

  const topicPills = useMemo(
    () => (uiLang === "bn" ? cfg.topicsBn : cfg.topicsEn),
    [uiLang, cfg],
  );
