...
tenant > compliance > global_law at equal similarity.

From repo root:
  python embedding/chroma_query_engine.py \"salary deduction\" --role worker --factory risky --top-k 5
"""
...
DEFAULT_CHROMA_DIR = REPO_ROOT / "embedding" / "chroma_data"
...
class ChromaQueryEngine:
        ...
        *,
        ...
        if not self.persist_dir.is_dir() or not meta_path.is_file():
            raise FileNotFoundError(
                f"Chroma index not found. Run: python embedding/build_chroma.py\n"
    ...
    def search(
        ...
        *,
        ...
        from embedding.roles import Role, role_may_access
...
def main() -> int:
    ...
    parser.add_argument("--persist-dir", type=Path, default=DEFAULT_CHROMA_DIR)
