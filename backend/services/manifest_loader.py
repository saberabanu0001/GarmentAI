...
from backend.core.security import roles_to_chroma_metadata, validate_role_list
...
def load_manifest(path: Path | None = None) -> list[ManifestRule]:
    p = path or get_settings().manifest_path
    raw = _load_raw(p)
...
def resolve_manifest_rule(chunks_filename: str, rules: list[ManifestRule]) -> ManifestRule:
    ...
    for rule in rules:
        if fnmatch.fnmatch(name, rule.pattern):
            return rule
    raise ValueError(
        f"No manifest rule matches {name!r}. Add a pattern to collection_manifest.yaml."
    )
