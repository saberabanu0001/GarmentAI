"""Build ChromaDB collections from data/chunked/*_chunks.txt (E5, cosine)."""
...
from backend.services.manifest_loader import chunk_uid, load_manifest, resolve_manifest_rule
from backend.services.parse_chunks import parse_chunks_file, passage_for_embedding
    ...
    *,
    ...
    for path in txt_files:
        ...
        if not records:
            continue
        print(
            f"Embedding {path.name} → {rule.collection} ({len(records)} chunks) …",
            flush=True,
        )
...
def main() -> int:
    ...
    parser.add_argument("--chunks-dir", type=Path, default=s.chunks_dir)
    parser.add_argument("--persist-dir", type=Path, default=s.chroma_dir)
    parser.add_argument("--manifest", type=Path, default=s.manifest_path)
