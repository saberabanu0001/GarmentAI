...
=======================
Preprocessing pipeline for JUKI DU-1181N Sewing Machine Manual
Workflow: PDF → Text Extraction → Cleaning → Chunking → Save

Source : JUKI DU-1181N Manual _ ManualsLib.pdf
Pages  : 24 total | skip first 2 (nav header + TOC)
Output : processed/machine_manual_chunks.txt
"""
...
# CONFIGURATION
# ─────────────────────────────────────────────
PDF_PATH   = "raw/JUKI DU-1181N Manual _ ManualsLib.pdf"
OUTPUT_TXT = "processed/machine_manual_chunks.txt"
SOURCE     = "Machine Manual"
CHUNK_SIZE = 1000
OVERLAP    = 100

# Pages to skip (0-indexed):
#   0-1 = ManualsLib website header + TOC (pages 1-2)
SKIP_PAGES = set(range(0, 2))
...
def clean_text(text):
    # Remove ManualsLib navigation header lines
    text = re.sub(r'Manuals\s*/\s*Brands\s*/.*\n?', '', text, flags=re.IGNORECASE)
    text = re.sub(r'JUKI\s+DU-\d+[^\n]*ManualsLib[^\n]*\n?', '', text, flags=re.IGNORECASE)
